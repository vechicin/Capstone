...
let createElementWithClass = (type, className) => {
  let element = document.createElement(type);
  element.classList.add(className);
  return element;
};

let featuredSpeakers = (speakers, number) => {
  for (let i = 0; i < number; i++) {
    let desktopSpeaker = document.querySelector('.desktop-speaker');
    let speakerDiv = createElementWithClass('div', 'speaker');
    let speakerImage = createElementWithClass('img', 'speaker-image')
    speakerImage.src = speakers[i].featuredImage;
    speakerImage.alt = speakers[i].alt;
    let speakerInfo = createElementWithClass('div', 'speaker-info');
    let h3 = createElementWithClass('h3', 'speaker-name');
    h3.textContent = speakers[i].name;
    let h4 = createElementWithClass('h4', 'speaker-role');
    h4.textContent = speakers[i].role;
    let p = createElementWithClass('p', 'speaker-description');
